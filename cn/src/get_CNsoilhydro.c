...

{
  ...
  for(band = 0; band < Nbands; band++)
      ...
      for(nidx = 0; nidx < Nnodes + 2; nidx++)
	{
	  ...
	  if(nidx < 2)
	    {
	    cn[band].moist[nidx] *= RHO_W;
	    }
	  else
	    {
	    cn[band].moist[nidx] *= (soil_con->dz_node[nidx-2] * RHO_W);
	    cn[band].ice[nidx] *= (soil_con->dz_node[nidx-2] * ice_density);
	    }
	}
	  ...
	  for(nidx = 0; nidx < Nnodes; nidx++)
	    {
		 ...
		 nidx != 0 && lidx != Nlayers - 1)
		  cn[band].rootfr[iveg][nidx] = (veg_con[iveg].root[lidx] + \
					      veg_con[iveg].root[lidx+1]) / \
		    2.0;
	      else
		cn[band].rootfr[iveg][nidx] = veg_con[iveg].root[lidx];
