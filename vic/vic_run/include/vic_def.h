...
#define MAX_PFT         20     /**< maximum number of CN PFTs */
#define NUMRAD          2      /**< number of radiation bands in CN */
...
enum
{
    ...
    OUT_CFAST,            /* fast soil C pool (g C/m^2) */
    OUT_CSLOWEST,         /* slowest soil C pool (g C/m^2) */
    OUT_CLITTERLAB,       /* litter labile C pool (g C/m^2) */
    OUT_CLITTERCELL,      /* litter cellulose C pool (g C/m^2) */
    OUT_CLITTERLIG,       /* litter lignin C pool (g C/m^2) */
    OUT_CCWD,             /* coarse woody debris C pool (g C/m^2) */
    OUT_CLEAF,            /* leaf C (g C/m^2) */
    OUT_CFROOT,           /* fine root C (g C/m^2) */
    OUT_CLIVESTEM,        /* live stem C (g C/m^2) */
    OUT_CDEADSTEM,        /* dead stem C (g C/m^2) */
    OUT_CLIVECROOT,       /* live coarse root C (g C/m^2) */
    OUT_CDEADCROOT,       /* dead coarse root C (g C/m^2) */
    OUT_CWOOD,            /* wood C (g C/m^2) */
    OUT_NINTER,           /* Carbon density in intermediate pool [g C/m2d] */
    OUT_NSLOW,            /* Carbon density in slow pool [g C/m2d] */
    OUT_NFAST,            /* fast soil C pool (g C/m^2) */
    OUT_NSLOWEST,         /* slowest soil C pool (g C/m^2) */
    OUT_NSOILMIN,         /* soil mineral N pool (g C/m^2) */
    OUT_NLITTERLAB,       /* litter labile C pool (g C/m^2) */
    OUT_NLITTERCELL,      /* litter cellulose C pool (g C/m^2) */
    OUT_NLITTERLIG,       /* litter lignin C pool (g C/m^2) */
    OUT_NCWD,             /* coarse woody debris C pool (g C/m^2) */
    OUT_NLEAF,            /* leaf C (g C/m^2) */
    OUT_NFROOT,           /* fine root C (g C/m^2) */
    OUT_NLIVESTEM,        /* live stem C (g C/m^2) */
    OUT_NDEADSTEM,        /* dead stem C (g C/m^2) */
    OUT_NLIVECROOT,       /* live coarse root C (g C/m^2) */
    OUT_NDEADCROOT,       /* dead coarse root C (g C/m^2) */
    OUT_CVEG,             /* total vegetation C (g C/m^2) */
    OUT_CSOIL,            /* total soil organic C (g C/m^2) */
    OUT_AUTORESP,         /* autotrophic respiration (g C/m^2/s) */
    OUT_NEP,              /* net ecosystem production (g C/m^2/s) */
    OUT_CN_LAIVEG,        /* leaf area index of veg covers */
    // Last value of enum - DO NOT ADD ANYTHING BELOW THIS LINE!!
    // used as a loop counter and must be >= the largest value in this enum
...
/******************************************************************************
 * @brief   Codes for carbon model option.
 *****************************************************************************/
enum
  {
    NO_CARBON,
...
typedef struct {
    ...
    bool CARBON;
  unsigned short int CARBON_MODEL;  /* Carbon model type, MAB 8/27/15 */
