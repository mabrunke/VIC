     ...
     char *argv[])
{
    ...
    cn_data_struct       *cn;   /* Added by MAB, 8/18/15 */
    ...
    ************************************/
    ...
    while (!MODEL_DONE) {
	    ...
	    if(options.CARBON_MODEL == CN_NORMAL || options.CARBON_MODEL \
		   == CN_ADECOMP)
	      {
		create_cn_output_list(out_data, veg_con[0].vegetat_type_num);
		set_cn_output_defaults(out_data_files, out_data);
	      }
		...
		nveg = calloc(1, sizeof(int));
		npfts = calloc(1, sizeof(int));
		if(options.CARBON_MODEL == CN_NORMAL || options.CARBON_MODEL \
		   == CN_ADECOMP)
		  {
		  begg = 1;
		  ...
		  nveg[0] = veg_con[0].vegetat_type_num;
		  npfts[0] = 0;
		  ...
		  clm_initialize2_(&dt, &nlevgrnd, &begg, &endg, &begc, \
                ...
                ErrorFlag = put_data(&all_vars, &atmos[0], &soil_con, veg_con,
                                     veg_lib, cn, &lake_con, out_data, &save_data,
                                     -global_param.nrecs);
...
>>>>>>> upstream/driver/cesm
                    ...
                    **************************************************/
                    write_flag = put_data(&all_vars, &atmos[rec], &soil_con, \
                                          veg_con, veg_lib, cn, &lake_con, \
					  out_data, &save_data, rec);
