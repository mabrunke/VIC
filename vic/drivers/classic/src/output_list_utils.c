...
create_output_list()
{
  ...
  strcpy(out_data[OUT_NEP].varname,"OUT_NEP");                             /* net ecosystem production (g C/m^2/s) */
    ...
    init_output_list(out_data, false, "%.4f", OUT_TYPE_FLOAT, 1, 1, \
		     N_OUTVAR_TYPES - 1);
...
/******************************************************************************
 * @brief    This routine creates OUT_CN_LAIVEG variables.
 *****************************************************************************/
void
create_cn_output_list(out_data_struct *out_data, int nveg)
{
    extern option_struct options;
    ...
    // Allocate space for data
    out_data[OUT_CN_LAIVEG].data = (double *) \
      calloc(out_data[OUT_CN_LAIVEG].nelem, sizeof(double));
    out_data[OUT_CN_LAIVEG].aggdata =
            (double *) calloc(out_data[OUT_CN_LAIVEG].nelem, sizeof(double));

    // Initialize data values
    init_output_list(out_data, false, "%.4f", OUT_TYPE_FLOAT, 1, \
		     N_OUTVAR_TYPES, N_OUTVAR_TYPES);
		 ...
		 int              varend)
{
    ...
    for (varid = varbeg; varid < varend; varid++) {
