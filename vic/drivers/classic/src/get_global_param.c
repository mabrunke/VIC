        ...
        if (cmdstr[0] != '#' && cmdstr[0] != '\n' && cmdstr[0] != '\0') {
            ...
            *************************************/
            if (strcasecmp("NLAYER", optstr) == 0) {
                ...
                if (strcasecmp("TRUE", flgstr) == 0) {
                    options.CARBON = true;
		    options.CARBON_MODEL = VIC_CARBON;
                }
                else {
                    options.CARBON = false;
		    options.CARBON_MODEL = NO_CARBON;
                }
            }
	    else if (strcasecmp("CARBON_MODEL", optstr) == 0) {
                sscanf(cmdstr, "%*s %s", flgstr);
                if (strcasecmp("VIC_CARBON", flgstr) == 0) {
                    options.CARBON_MODEL = NO_CARBON;
                }
		else if (strcasecmp("CN_NORMAL", flgstr) == 0) {
                    options.CARBON_MODEL = CN_NORMAL;
                }
		else if (strcasecmp("CN_ADECOMP", flgstr) == 0) {
                    options.CARBON_MODEL = CN_ADECOMP;
                }
                else {
                    options.CARBON_MODEL = NO_CARBON;
                }
		if(options.CARBON == false)
		  options.CARBON_MODEL = NO_CARBON;
            }
