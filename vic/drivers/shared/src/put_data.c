    ...
    for (veg = 0; veg <= veg_con[0].vegetat_type_num; veg++) {
        ...
        if (Cv > 0) {
            ...
            *********************************/
		    ...
		    if(options.CARBON_MODEL == CN_NORMAL || \
		       options.CARBON_MODEL == CN_ADECOMP)
		      out_data[OUT_CN_LAIVEG].data[veg] += \
			veg_var[veg][band].LAI * ThisAreaFract * \
			ThisTreeAdjust * (1.0 - Clake);
    ...
    *****************************************/
    ...
    for (index = 0; index < options.Nlayer; index++) {
        ...
        out_data[OUT_DELSOILMOIST].data[0] +=
            out_data[OUT_SOIL_MOIST].data[index];
        log_warn("Duct tape begins here: %f %f",
                 out_data[OUT_SOIL_LIQ].data[index],
                 out_data[OUT_SOIL_MOIST].data[index]);
