...

{
  ...
  /* Gett VIC time step */
  dt = (double) global_param->dt;

  /* Get current date from dmy */
  year = dmy[rec].year;
  month = dmy[rec].month;
  day = dmy[rec].day;
  secs = dmy[rec].dayseconds;
  ...
  get_CNveg(Nbands, Nveg, atmos->prec[NR], veg_con, veg_lib, all_vars, cn);
