...
! !USES:
  ...
  use clmtype
  use clm_varpar  , only : maxpatch_pft, nlevsno, numrad, nlevlak, &
                           numpft, mxpft, ndst, nvoc, nlevurb, nlevsoi, mxveg
!
! !PUBLIC TYPES:
  implicit none
  save
...
!
! !INTERFACE:
  subroutine initClmtype(begg, endg, begc, endc, begp, endp, nveg, vegtype, &
...
! !ARGUMENTS:
    ...
    integer  :: vegtype(1:endg*(mxveg+1))          ! VIC vegetation types
    real(r8) :: vegfrac(1:endg*(mxveg+1))          ! VIC vegetation fractions
!    real(r8) :: fpft(1:mxveg+1,0:mxpft)     ! PFT fractions ingested
    integer  :: nveg(begg:endg)                  ! # VIC vegetation types
    integer :: npfts(begg:endg)                  ! # CLM PFTs
...
!    integer :: nump         ! total number of pfts across all processors
    integer :: ityp          ! CLM vegetation type
    integer :: p,c,g, m      ! indices
    real(r8) :: pftfrac(endg,0:mxpft)      ! CLM PFT fractions
    real(r8) :: pftsum                ! Sum of CLM PFT fractions
!------------------------------------------------------------------------
    ...
    do g = 1, endg
     ...
     do p = 1, nveg(g)
!	print *, g, p, vegfrac((g-1)*(mxveg+1) + p), vegtype(p)
!      do m = 0, mxpft
!        print *, p, m, vegfrac(p), fpft(p,m)
!        pftfrac(m) = pftfrac(m) + vegfrac(p) * fpft(p,m)
!      end do
      if(vegtype((g-1)*(mxveg+1) + p) .eq. 0) then
        m = 2
!        m = 1
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p)
        npfts(g) = npfts(g) + 1
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 1) then
        m = 5
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p)
        npfts(g) = npfts(g) + 1
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 2) then
        m = 3
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p)
        npfts(g) = npfts(g) + 1
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 3) then
        m = 8
!        m = 7
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p)
        npfts(g) = npfts(g) + 1
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 4) then
        m = 2
!        m = 1
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.5
...
!        m = 7
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.5
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 5) then
        m = 2
!        m = 1
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.4
...
!        m = 7
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.1
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.1
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 6) then
        m = 14
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.3

        m = 12
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.2
...
!        m = 7
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.2
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 7) then
        m = 9
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.1
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 8) then
        m = 9
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.05

        m = 13
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.15
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.1
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 9) then
        m = 14
        if(pftfrac(g,m) .eq. 0.) then
          npfts(g) = npfts(g) + 1
        end if
        ...
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p) * 0.4
      else if(vegtype((g-1)*(mxveg+1) + p) .eq. 10) then
        m = 17
        pftfrac(g,m) = pftfrac(g,m) + vegfrac((g-1)*(mxveg+1) + p)
        npfts(g) = npfts(g) + 1
      else
    ...
    pftsum = 0.
    do m = 0, mxpft
!      print *, m, pftfrac(g,m), npfts(g)
!      if(pftfrac(m) .gt. 0.) then
!        npfts = npfts + 1
!      end if
      pftsum = pftsum + pftfrac(g,m)
    enddo

    if(pftsum > 1.) then
      do m = 0, mxpft
        pftfrac(g,m) = pftfrac(g,m) / pftsum
      enddo
      pftsum = 1.
    end if

    endp = endp + endc * npfts(g)

    call init_pft_type     (begp, endp, clm3%g%c%p)

    do c = begc, endc
      p = 1
      do m = 0, mxpft
        if(pftfrac(g,m) .gt. 0.) then
          clm3%g%c%p%gridcell((c-1)*npfts(g) + p) = 1
          clm3%g%c%p%wtgcell((c-1)*npfts(g) + p) = pftfrac(g,m) / endc
          clm3%g%c%p%column((c-1)*npfts(g) + p) = c
          clm3%g%c%p%wtcol((c-1)*npfts(g) + p) = pftfrac(g,m) / endc
          clm3%g%c%p%itype((c-1)*npfts(g) + p) = m
          clm3%g%c%p%mxy((c-1)*npfts(g) + p) = m
          p = p + 1
        endif
      enddo
    enddo
   enddo
    ...
    endc = endg * endc
    call init_column_type  (begc, endc, clm3%g%c)

    do c = begc, endc
      clm3%g%c%gridcell(c) = 1.
      clm3%g%c%wtgcell(c) = 1. / endc
      clm3%g%c%pfti(c) = 1
      clm3%g%c%pftf(c) = npfts(g)
      clm3%g%c%npfts(c) = npfts(g)
      clm3%g%c%itype(c) = 1
    enddo

    call init_gridcell_type(begg, endg, clm3%g, clm_a2l)

    do g = begg, endg
      clm3%g%coli(g) = begc
      clm3%g%colf(g) = endc
      clm3%g%ncolumns(g) = endc
      clm3%g%pfti(g) = 1
      clm3%g%pftf(g) = npfts(g)
      clm3%g%npfts(g) = npfts(g)
    enddo
...
!#endif
    ...
    call init_carbon_balance_type(begp, endp, clm3%g%c%p%pcbal)
    call init_carbon_balance_type(begc, endc, clm3%g%c%ccbal)

    ! nitrogen balance structures (pft and column levels)

    call init_nitrogen_balance_type(begp, endp, clm3%g%c%p%pnbal)
    call init_nitrogen_balance_type(begc, endc, clm3%g%c%cnbal)

    ! pft physical state variables at pft level and averaged to the column

    call init_pft_pstate_type(nlevgrnd, begp, endp, clm3%g%c%p%pps)
    call init_pft_pstate_type(nlevgrnd, begc, endc, clm3%g%c%cps%pps_a)

    ! pft ecophysiological variables (only at the pft level for now)
    call init_pft_epv_type(begp, endp, clm3%g%c%p%pepv)
...
!#endif
    call init_pft_vstate_type(begp, endp, clm3%g%c%p%pvs)

    ! pft energy state variables at the pft level and averaged to the column

    call init_pft_estate_type(begp, endp, clm3%g%c%p%pes)
    call init_pft_estate_type(begc, endc, clm3%g%c%ces%pes_a)

    ! pft water state variables at the pft level and averaged to the column

    call init_pft_wstate_type(begp, endp, clm3%g%c%p%pws)
    call init_pft_wstate_type(begc, endc, clm3%g%c%cws%pws_a)

    ! pft carbon state variables at the pft level and averaged to the column

    call init_pft_cstate_type(begp, endp, clm3%g%c%p%pcs)
    call init_pft_cstate_type(begc, endc, clm3%g%c%ccs%pcs_a)

    ! pft nitrogen state variables at the pft level and averaged to the column

    call init_pft_nstate_type(begp, endp, clm3%g%c%p%pns)
    call init_pft_nstate_type(begc, endc, clm3%g%c%cns%pns_a)

    ! pft energy flux variables at pft level and averaged to column

    call init_pft_eflux_type(begp, endp, clm3%g%c%p%pef)
    call init_pft_eflux_type(begc, endc, clm3%g%c%cef%pef_a)

    ! pft water flux variables

    call init_pft_wflux_type(begp, endp, clm3%g%c%p%pwf)
    call init_pft_wflux_type(begc, endc, clm3%g%c%cwf%pwf_a)

    ! pft carbon flux variables at pft level and averaged to column

    call init_pft_cflux_type(begp, endp, clm3%g%c%p%pcf)
    call init_pft_cflux_type(begc, endc, clm3%g%c%ccf%pcf_a)

    ! pft nitrogen flux variables at pft level and averaged to column

    call init_pft_nflux_type(begp, endp, clm3%g%c%p%pnf)
    call init_pft_nflux_type(begc, endc, clm3%g%c%cnf%pnf_a)

    ! pft VOC flux variables at pft level and averaged to column

!    call init_pft_vflux_type(begp, endp, clm3%g%c%p%pvf)
!    call init_pft_vflux_type(begc, endc, clm3%g%c%cvf%pvf_a)
...
!    call init_gridcell_efstate_type(begg, endg, clm3%g%gve)
    ...
    call init_column_pstate_type(nlevgrnd, begc, endc, clm3%g%c%cps)
    call init_column_pstate_type(nlevgrnd, begg, endg, clm3%g%gps%cps_a)
    call init_column_pstate_type(nlevgrnd, 1,       1, clm3%mps%cps_a)

    ! column energy state variables at column level and averaged to
    ! the landunit and gridcell and model

    call init_column_estate_type(nlevgrnd, begc, endc, clm3%g%c%ces)
    call init_column_estate_type(nlevgrnd, begg, endg, clm3%g%ges%ces_a)
    call init_column_estate_type(nlevgrnd, 1,       1, clm3%mes%ces_a)

    ! column water state variables at column level and averaged to
    ! the landunit and gridcell and model

    call init_column_wstate_type(nlevgrnd, begc, endc, clm3%g%c%cws)
    call init_column_wstate_type(nlevgrnd, begg, endg, clm3%g%gws%cws_a)
    call init_column_wstate_type(nlevgrnd, 1,       1, clm3%mws%cws_a)

    ! column carbon state variables at column level and averaged to
    ! the landunit and gridcell and model

    call init_column_cstate_type(begc, endc, clm3%g%c%ccs)
    ...
    call init_column_nstate_type(begc, endc, clm3%g%c%cns)
    ...
    call init_column_wflux_type(begc, endc, clm3%g%c%cwf)
    ...
    call init_column_cflux_type(begc, endc, clm3%g%c%ccf)

    ! column nitrogen flux variables at column level

    call init_column_nflux_type(begc, endc, clm3%g%c%cnf)
...
!
! !INTERFACE:
  subroutine init_gridcell_type (beg, end,g,a2l)
!
! !DESCRIPTION:
...
!------------------------------------------------------------------------
   ...
   ! Initialize atm2lnd surface pressure, MAB 8/14/13
   !                    radiation, MAB 8/16/13
   ...
   allocate(a2l%forc_solad(beg:end,1:numrad))
   allocate(a2l%forc_solai(beg:end,1:numrad))
...
!
! !INTERFACE:
  subroutine init_pft_pstate_type(nlevgrnd, beg, end, pps)
!
! !DESCRIPTION:
! Initialize pft physical state
!
...
!
! !INTERFACE:
  subroutine init_column_pstate_type(nlevgrnd, beg, end, cps)
!
! !DESCRIPTION:
...
!
! !INTERFACE:
  subroutine init_column_estate_type(nlevgrnd, beg, end, ces)
!
! !DESCRIPTION:
...
!
! !INTERFACE:
  subroutine init_column_wstate_type(nlevgrnd, beg, end, cws)
!
! !DESCRIPTION:
