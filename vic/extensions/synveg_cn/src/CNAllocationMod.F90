...
! 10/10/12: Revised for use in VIC by Michael Brunke
!
!EOP
!-----------------------------------------------------------------------
...
!
! !INTERFACE:
subroutine CNAllocationInit ( lbc, ubc, lbp, ubp, dt )
!
! !DESCRIPTION:
!
! !USES:
...
! 10/24/12: Adapted for use in VIC by Michael Brunke
!
! !LOCAL VARIABLES:
...
!
! !INTERFACE:
subroutine CNAllocation (dt, lbp, ubp, lbc, ubc, num_soilc, num_soilp)
!
! !DESCRIPTION:
!
! !USES:
...
! 10/10/12: Revised for use in VIC by Michael Brunke
! 02/06/14: Added ignoring crops in PFT loops (Michael Brunke).
!
! !LOCAL VARIABLES:
...
!-----------------------------------------------------------------------
   ...
   leaf_curmr                  => clm3%g%c%p%pcf%leaf_curmr
   froot_curmr                 => clm3%g%c%p%pcf%froot_curmr
   livestem_curmr              => clm3%g%c%p%pcf%livestem_curmr
   livecroot_curmr             => clm3%g%c%p%pcf%livecroot_curmr
   leaf_xsmr                   => clm3%g%c%p%pcf%leaf_xsmr
   froot_xsmr                  => clm3%g%c%p%pcf%froot_xsmr
   livestem_xsmr               => clm3%g%c%p%pcf%livestem_xsmr
   livecroot_xsmr              => clm3%g%c%p%pcf%livecroot_xsmr
   ...
   fcur2                       => pftcon%fcur
   gddmaturity                 => clm3%g%c%p%pps%gddmaturity
   huileaf                     => clm3%g%c%p%pps%huileaf
   huigrain                    => clm3%g%c%p%pps%huigrain
   hui                         => clm3%g%c%p%pps%gddplant
   leafout                     => clm3%g%c%p%pps%gddtsoi
   croplive                    => clm3%g%c%p%pps%croplive
   peaklai                     => clm3%g%c%p%pps%peaklai
   ...
   cpool_to_xsmrpool           => clm3%g%c%p%pcf%cpool_to_xsmrpool
   psnsun_to_cpool             => clm3%g%c%p%pcf%psnsun_to_cpool
   psnshade_to_cpool           => clm3%g%c%p%pcf%psnshade_to_cpool
   cpool_to_leafc              => clm3%g%c%p%pcf%cpool_to_leafc
   cpool_to_leafc_storage      => clm3%g%c%p%pcf%cpool_to_leafc_storage
   cpool_to_frootc             => clm3%g%c%p%pcf%cpool_to_frootc
   cpool_to_frootc_storage     => clm3%g%c%p%pcf%cpool_to_frootc_storage
   cpool_to_livestemc          => clm3%g%c%p%pcf%cpool_to_livestemc
   cpool_to_livestemc_storage  => clm3%g%c%p%pcf%cpool_to_livestemc_storage
   cpool_to_deadstemc          => clm3%g%c%p%pcf%cpool_to_deadstemc
   cpool_to_deadstemc_storage  => clm3%g%c%p%pcf%cpool_to_deadstemc_storage
   cpool_to_livecrootc         => clm3%g%c%p%pcf%cpool_to_livecrootc
   cpool_to_livecrootc_storage => clm3%g%c%p%pcf%cpool_to_livecrootc_storage
   cpool_to_deadcrootc         => clm3%g%c%p%pcf%cpool_to_deadcrootc
   cpool_to_deadcrootc_storage => clm3%g%c%p%pcf%cpool_to_deadcrootc_storage
   cpool_to_gresp_storage      => clm3%g%c%p%pcf%cpool_to_gresp_storage
   cpool_to_grainc             => clm3%g%c%p%pcf%cpool_to_grainc
   cpool_to_grainc_storage     => clm3%g%c%p%pcf%cpool_to_grainc_storage
   npool_to_grainn             => clm3%g%c%p%pnf%npool_to_grainn
   npool_to_grainn_storage     => clm3%g%c%p%pnf%npool_to_grainn_storage
   retransn_to_npool           => clm3%g%c%p%pnf%retransn_to_npool
   sminn_to_npool              => clm3%g%c%p%pnf%sminn_to_npool
   npool_to_leafn              => clm3%g%c%p%pnf%npool_to_leafn
   npool_to_leafn_storage      => clm3%g%c%p%pnf%npool_to_leafn_storage
   npool_to_frootn             => clm3%g%c%p%pnf%npool_to_frootn
   npool_to_frootn_storage     => clm3%g%c%p%pnf%npool_to_frootn_storage
   npool_to_livestemn          => clm3%g%c%p%pnf%npool_to_livestemn
   npool_to_livestemn_storage  => clm3%g%c%p%pnf%npool_to_livestemn_storage
   npool_to_deadstemn          => clm3%g%c%p%pnf%npool_to_deadstemn
   npool_to_deadstemn_storage  => clm3%g%c%p%pnf%npool_to_deadstemn_storage
   npool_to_livecrootn         => clm3%g%c%p%pnf%npool_to_livecrootn
   npool_to_livecrootn_storage => clm3%g%c%p%pnf%npool_to_livecrootn_storage
   npool_to_deadcrootn         => clm3%g%c%p%pnf%npool_to_deadcrootn
   npool_to_deadcrootn_storage => clm3%g%c%p%pnf%npool_to_deadcrootn_storage
   fpi                         => clm3%g%c%cps%fpi
   fpg                         => clm3%g%c%cps%fpg
   potential_immob             => clm3%g%c%cnf%potential_immob
   actual_immob                => clm3%g%c%cnf%actual_immob
   sminn_to_plant              => clm3%g%c%cnf%sminn_to_plant
   sminn_to_denit_excess       => clm3%g%c%cnf%sminn_to_denit_excess
   supplement_to_sminn         => clm3%g%c%cnf%supplement_to_sminn
   aleafi                      => clm3%g%c%p%pps%aleafi
   astemi                      => clm3%g%c%p%pps%astemi
   aleaf                       => clm3%g%c%p%pps%aleaf
   astem                       => clm3%g%c%p%pps%astem
...
!      print *, p, ivt(p), psnsun(p), laisun(p), psnsha(p), laisha(p)
!      print *, p, ivt(p), psnsun_to_cpool(p), psnshade_to_cpool(p), gpp(p)
...
!      print *, p, ivt(p), avail_retransn(p), annmax_retransn(p), gpp(p), &
!	annsum_potential_gpp(p)
...
!      print *, p, ivt(p), avail_retransn(p), retransn(p), plant_ndemand(p), &
!	retransn_to_npool(p)
   ...
   call p2c_1d(lbp,ubp,lbc,ubc,plant_ndemand,col_plant_ndemand,'unity')
...
!      print *, p, ivt(p), sminn_to_npool(p), retransn_to_npool(p), &
!	plant_nalloc(p)
...
!      print *, p, ivt(p), cpool_to_leafc(p), nlc, fcur, plant_calloc(p), &
!	c_allometry(p), plant_nalloc(p), n_allometry(p)
...
!      end if
!	if(p .eq. 2) then
!          print *, 'CNAllocation', cpool_to_leafc(p), nlc, fcur, &
!		plant_calloc(p), c_allometry(p)
!        end if
...
!	print *, p, ivt(p), npool_to_leafn(p), nlc, cnl, fcur
      if (woody(ivt(p)) == 1._r8) then
