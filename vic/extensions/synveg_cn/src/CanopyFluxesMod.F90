...
!
! !INTERFACE:
  subroutine CanopyFluxes(dtime, nlevgrnd, lbg, ubg, lbc, ubc, lbp, ubp)
!
! !DESCRIPTION:
...
! !ARGUMENTS:
    ...
    integer, intent(in) :: nlevgrnd              ! # soil layers
...
! 20 August 2013: Michael Brunke; Adapted for use in VIC.
! 7 February 2014: Added ignoring crops in PFT loops (Michael Brunke).
!
! !LOCAL VARIABLES:
!
...
!   forc_rho       => clm_a2l%forc_rho
   ...
   btran          => clm3%g%c%p%pps%btran
   rootfr         => clm3%g%c%p%pps%rootfr
   rootr          => clm3%g%c%p%pps%rootr
   rresis         => clm3%g%c%p%pps%rresis
   emv            => clm3%g%c%p%pps%emv
   ...
   ram1           => clm3%g%c%p%pps%ram1
   htop           => clm3%g%c%p%pps%htop
   ...
   fdry           => clm3%g%c%p%pps%fdry
   laisun         => clm3%g%c%p%pps%laisun
   laisha         => clm3%g%c%p%pps%laisha
!   qflx_tran_veg  => clm3%g%c%p%pwf%qflx_tran_veg
   fwet           => clm3%g%c%p%pps%fwet
   h2ocan         => clm3%g%c%p%pws%h2ocan
   dt_veg         => clm3%g%c%p%pps%dt_veg
   sabv           => clm3%g%c%p%pef%sabv
!   qflx_evap_veg  => clm3%g%c%p%pwf%qflx_evap_veg
!   eflx_sh_veg    => clm3%g%c%p%pef%eflx_sh_veg
!   taux           => clm3%g%c%p%pmf%taux
!   tauy           => clm3%g%c%p%pmf%tauy
!   eflx_sh_grnd   => clm3%g%c%p%pef%eflx_sh_grnd
!   qflx_evap_soi  => clm3%g%c%p%pwf%qflx_evap_soi
!   t_ref2m        => clm3%g%c%p%pes%t_ref2m
!   q_ref2m        => clm3%g%c%p%pes%q_ref2m
!   t_ref2m_r      => clm3%g%c%p%pes%t_ref2m_r
!   rh_ref2m_r     => clm3%g%c%p%pes%rh_ref2m_r
!   rh_ref2m       => clm3%g%c%p%pes%rh_ref2m
   dlrad          => clm3%g%c%p%pef%dlrad
   ulrad          => clm3%g%c%p%pef%ulrad
   cgrnds         => clm3%g%c%p%pef%cgrnds
   cgrndl         => clm3%g%c%p%pef%cgrndl
   cgrnd          => clm3%g%c%p%pef%cgrnd
   fpsn           => clm3%g%c%p%pcf%fpsn
   forc_hgt_u_pft => clm3%g%c%p%pps%forc_hgt_u_pft
   thm            => clm3%g%c%p%pes%thm
   irrig_rate         => clm3%g%c%cps%irrig_rate
   n_irrig_steps_left => clm3%g%c%cps%n_irrig_steps_left
   ...
   fn = 0
   do p = lbp, ubp
      if (frac_veg_nosno(p) /= 0 .and. ivt(p) < npcropmin) then
         fn = fn + 1
         filterp(fn) = p
      end if
   end do
...
!  commented out for now, MAB 8/29/13
!   do f = 1, fn
!      p = filterp(f)
!      c = pcolumn(p)
...
!      cir(p) =   emv(p)*emg(c)*sb
      ...
      ! Find these as percentage of surface pressure, MAB 8/20/13
...
!         end if
 ...
 !        raw(p,2) = rah(p,2)
      ...
      call Stomata (fn, filterp, lbp, ubp, svpts, eah, o2, co2, rb, dayl_factor, 'sun')
      call Stomata (fn, filterp, lbp, ubp, svpts, eah, o2, co2, rb, dayl_factor, 'sha')
...
!------------------------------------------------------------------------------
     ...
     pgridcell => clm3%g%c%p%gridcell
     ivt       => clm3%g%c%p%itype
     tl        => clm3%g%c%p%pes%t_veg
     btran     => clm3%g%c%p%pps%btran
     if (phase == 'sun') then
        apar   => clm3%g%c%p%pef%parsun
        rs     => clm3%g%c%p%pps%rssun
        psn    => clm3%g%c%p%pcf%psnsun
        ci     => clm3%g%c%p%pps%cisun
        sla    => clm3%g%c%p%pps%slasun
        lnc    => clm3%g%c%p%pps%lncsun   
        vcmx   => clm3%g%c%p%pps%vcmxsun   
     else if (phase == 'sha') then
        apar   => clm3%g%c%p%pef%parsha
        rs     => clm3%g%c%p%pps%rssha
        psn    => clm3%g%c%p%pcf%psnsha
        ci     => clm3%g%c%p%pps%cisha
        sla    => clm3%g%c%p%pps%slasha   
        lnc    => clm3%g%c%p%pps%lncsha   
        vcmx   => clm3%g%c%p%pps%vcmxsha
     end if
     ...
     do f = 1, fn
	   ...
	   print *, p, ivt(p), phase, cp, kc, ko, o2(p)
...
!           print *, p, ivt(p), phase, psn(p), wj, wc, we, ci(p), cp
  ...
  real*8 function f1(ab, bc)

     use shr_kind_mod , only : r8 => shr_kind_r8

     real(r8), intent(in) :: ab
     real(r8), intent(in) :: bc

     f1 = ab**((bc-25._r8)/10._r8)

  return
  end function f1

  real*8 function f2(ab)
     ...
     f2 = 1._r8 + exp((-2.2e05_r8+710._r8*(ab+SHR_CONST_TKFRZ))/(SHR_CONST_RGAS*0.001_r8*(ab+SHR_CONST_TKFRZ)))
