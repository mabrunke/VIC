...
int
redistribute_during_storm(cell_data_struct ***cell,
    ...
    for (layer = 0; layer < options.Nlayer; layer++) {
        for (band = 0; band < options.SNOW_BAND; band++) {
            ...
            for (frost_area = 0; frost_area < options.Nfrost; frost_area++) {
                ...
                if (error) {
                    fprintf(stderr,
                            "%s: Error in ice accounting %f -> %f record %i\n",
                            __FILE__,
                            cell[WET][veg][band].layer[layer].ice[frost_area] *
                            old_mu +
                            cell[DRY][veg][band].layer[layer].ice[frost_area] *
                            (1. -
                             old_mu), temp_wet * new_mu + temp_dry *
                            (1. - new_mu), rec);
                    return(ERROR);
                }
    ...
    ****************************************/
    if (veg < Nveg) {
        for (band = 0; band < options.SNOW_BAND; band++) {
            ...
            if (error) {
                fprintf(stderr,
                        "%s: Error in Wdew accounting %f -> %f record %i\n",
...
unsigned char
redistribute_moisture_for_storm(double *wet_value,
                                double *dry_value,
                                double  max_moist,
                                double  old_mu,
                                double  new_mu)
