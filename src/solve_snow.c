...
*********************************************************************/
    ...
    if ((snow->swq > 0 || snowfall[WET] > 0. ||
        ...
        if (mu != 1 && options.FULL_ENERGY) {
            fprintf(stderr,
                    "ERROR: Snow model cannot be used if mu (%f) is not equal to 1.\n\tsolve_snow.c: record = %i,\t vegetation type = %i",
                    mu, rec, iveg);
            return(ERROR);
        }
        else if (mu != 1) {
            fprintf(stderr,
                    "WARNING: Snow is falling, but mu not equal to 1 (%f)\n",
                    mu);
                ...
                ***********************************************/
                ...
                ErrorFlag = snow_intercept((double)dt * SECPHOUR, 1.,
                                           ...
                                           *UnderStory, band,
                                           ...
                                           layer_dry, layer_wet, soil_con,
                                           veg_var_dry, veg_var_wet);
                if (ErrorFlag == ERROR) {
                    return (ERROR);
                }
        ...
        if (snow->swq > 0.0 || snowfall[0] > 0) {
            ...
            ******************************/
            ...
            ErrorFlag = snow_melt((*Le), (*NetShortSnow), Tcanopy, Tgrnd,
                                  ...
                                  *LongUnderIn, pressure, rainfall[WET],
                                  snowfall[WET],
            ...
            if (snow->swq > 0.) {
                ...
                if (snow->surf_temp <= 0) {
                    ...
                    snow->density = snow_density(snow, snowfall[WET], old_swq,
