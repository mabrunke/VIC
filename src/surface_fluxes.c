...
**********************************************************************/
{
    ...
    if (options.CARBON) {
        for (dist = 0; dist < 2; dist++) {
            store_gsLayer[dist] = (double*)calloc(options.Ncanopy,
                                                  sizeof(double));
        }
    }
        ...
        if (options.CARBON && iveg < Nveg) {
            ...
            for (cidx = 0; cidx < options.Ncanopy; cidx++) {
                if (LAIlayer[cidx] > 1e-10) {
                    veg_var_wet->aPARLayer[cidx] =
                        (atmos->par[hidx] /
                         Epar) * faPAR[cidx] / LAIlayer[cidx];
                    veg_var_wet->aPAR += atmos->par[hidx] * faPAR[cidx] /
                                         LAIlayer[cidx];
                }
                else {
                    veg_var_wet->aPARLayer[cidx] = atmos->par[hidx] / Epar *
                                                   faPAR[cidx] / 1e-10;
...
// iter_snow_energy.sensible + iter_snow_energy.latent + iter_snow_energy.latent_sub + NetShortSnow + NetLongSnow + ( snow_grnd_flux + iter_snow_energy.advection - iter_snow_energy.deltaCC + iter_snow_energy.refreeze_energy + iter_snow_energy.advected_sensible ) * step_snow.coverage
                ...
                **************************************************/
                ...
                Tsurf = calc_surf_energy_bal((*Le), LongUnderIn, NetLongSnow,
                                             ...
                                             iter_layer[DRY], iter_layer[WET],
                                             &(iter_snow), soil_con,
                                             &iter_soil_veg_var[DRY],
                                             &iter_soil_veg_var[WET],
                                             gp->nrecs);
                ...
                *****************************************/
                ...
                if (fabs(tol_under - last_tol_under) < GRND_TOL && tol_under >
                    1.) {
                    tol_under = -999;
                }
            ...
            if (iveg < Nveg && !step_snow.snow && dryFrac > 0) {
                for (dist = 0; dist < Ndist; dist++) {
                    canopy_assimilation(veg_lib[veg_class].Ctype,
                                        ...
                                        atmos->Catm[hidx],
                                        CanopLayerBnd,
                                        veg_lib[veg_class].LAI[dmy[rec].month -
                                                               1],
                                        "rs",
        ...
        for (dist = 0; dist < Ndist; dist++) {
            if (iveg != Nveg) {
                ...
                if (options.CARBON) {
                    ...
                    for (cidx = 0; cidx < options.Ncanopy; cidx++) {
                        store_gsLayer[dist][cidx] += 1 /
                                                     soil_veg_var[dist].rsLayer[
                            cidx];
                    }
...
// energy->AtmosSensible + energy->AtmosLatent + energy->AtmosLatentSub + energy->NetShortAtmos + energy->NetLongAtmos + energy->grnd_flux + energy->deltaH + energy->fusion + energy->advection - energy->deltaCC + energy->refreeze_energy + energy->advected_sensible
    ...
    **********************************************************/
    ...
    if (options.CARBON && iveg != Nveg) {
        ...
        for (cidx = 0; cidx < options.Ncanopy; cidx++) {
            veg_var_wet->rsLayer[cidx] = 1 / store_gsLayer[WET][cidx] /
                                         (double)N_steps;
        }
        ...
        for (cidx = 0; cidx < options.Ncanopy; cidx++) {
            veg_var_dry->rsLayer[cidx] = 1 / store_gsLayer[DRY][cidx] /
                                         (double)N_steps;
        }
        ...
        if (veg_var_wet->NPP > 0) {
            veg_var_wet->AnnualNPP += veg_var_wet->NPP * MCg * 3600 * gp->dt;
        }
        if (veg_var_dry->NPP > 0) {
            veg_var_dry->AnnualNPP += veg_var_dry->NPP * MCg * 3600 * gp->dt;
        }
    }
