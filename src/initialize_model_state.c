...
int
initialize_model_state(dist_prcp_struct    *prcp,
                       dmy_struct           dmy,
    ...
    if (options.LAKES) {
        ...
        ErrorFlag =
            initialize_lake(lake_var, lake_con, soil_con,
                            &(cell[WET][tmp_lake_idx][0]), surf_temp, 0);
        if (ErrorFlag == ERROR) {
            return(ErrorFlag);
        }
    }
            ...
            for (veg = 0; veg <= Nveg; veg++) {
                for (band = 0; band < options.SNOW_BAND; band++) {
                    for (lidx = 0; lidx < options.Nlayer; lidx++) {
                        if (cell[dist][veg][band].layer[lidx].moist >
                            ...
                            for (frost_area = 0;
                                 ...
                                 frost_area++) {
                                cell[dist][veg][band].layer[lidx].ice[frost_area
                                ] *= soil_con->max_moist[lidx] /
                                     cell[dist][veg][band].layer[lidx].moist;
                            }
                            cell[dist][veg][band].layer[lidx].moist =
                        ...
                        for (frost_area = 0;
                             frost_area < options.Nfrost;
                             frost_area++) {
                            if (cell[dist][veg][band].layer[lidx].ice[frost_area
                                ] > cell[dist][veg][band].layer[lidx].moist) {
                                cell[dist][veg][band].layer[lidx].ice[frost_area
                                ] = cell[dist][veg][band].layer[lidx].moist;
                            }
        ...
        for (veg = 0; veg <= Nveg; veg++) {
            ...
            if (Cv > 0) {
                for (band = 0; band < options.SNOW_BAND; band++) {
                    for (lidx = 0; lidx < options.Nlayer; lidx++) {
                        moist[veg][band][lidx] =
                            cell[0][veg][band].layer[lidx].moist;
                    ...
                    for (dry = 0; dry < Ndist; dry++) {
                        for (lidx = 0; lidx < options.Nlayer; lidx++) {
                            ...
                            for (frost_area = 0;
                                 frost_area < options.Nfrost;
                                 frost_area++) {
                                cell[dry][veg][band].layer[lidx].ice[frost_area]
                                    = ice[veg][band][lidx][frost_area];
                            }
                        }
...
int
update_thermal_nodes(dist_prcp_struct *prcp,
                     int               Nveg,
                     int               Nnodes,
                     int               Ndist,
                     soil_con_struct  *soil_con,
                ...
                if (soil_con->AreaFract[band] > 0.) {
                    ...
                    for (dry = 0; dry < Ndist; dry++) {
                        if (!(options.LAKES && veg_con->LAKE != 0)) {
                            if (options.QUICK_FLUX) {
                                ErrorFlag =
                                    estimate_layer_ice_content_quick_flux(cell[
                                                                              dry][
                                                                              veg][
                                                                              band].layer,
                                                                          soil_con->depth, soil_con->dp,
