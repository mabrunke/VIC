...
*********************************************************************/
{
    ...
    if (feof(init_state)) {
        sprintf(ErrStr,
                "Requested grid cell (%d) is not in the model state file.",
                cellnum);
        nrerror(ErrStr);
    }
        ...
        for (band = 0; band < Nbands; band++) {
            ...
            for (dist = 0; dist < Ndist; dist++) {
                ...
                if (veg < Nveg) {
                    /* Read dew storage */
                    if (options.BINARY_STATE_FILE) {
                        if (fread(&veg_var[dist][veg][band].Wdew,
                                  sizeof(double), 1,
                                  init_state) != 1) {
                            nrerror("End of model state file found unexpectedly");
                        }
                        ...
                        if (fscanf(init_state, " %lf",
                                   &veg_var[dist][veg][band].Wdew) == EOF) {
                            nrerror("End of model state file found unexpectedly");
                        }
                    }
                    ...
                    if (options.CARBON) {
                        if (options.BINARY_STATE_FILE) {
                            ...
                            if (fread(&(veg_var[dist][veg][band].AnnualNPP),
                                      sizeof(double), 1, init_state) != 1) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            if (fread(&(veg_var[dist][veg][band].AnnualNPPPrev),
                                      sizeof(double), 1, init_state) != 1) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            /* Read Soil Carbon Storage */
                            if (fread(&(cell[dist][veg][band].CLitter),
                                      sizeof(double), 1, init_state) != 1) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            if (fread(&(cell[dist][veg][band].CInter),
                                      sizeof(double), 1, init_state) != 1) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            if (fread(&(cell[dist][veg][band].CSlow),
                                      sizeof(double), 1, init_state) != 1) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            ...
                            /* Read cumulative annual NPP */
                            if (fscanf(init_state, " %lf",
                                       &veg_var[dist][veg][band].AnnualNPP) ==
                                EOF) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            if (fscanf(init_state, " %lf",
                                       &veg_var[dist][veg][band].AnnualNPPPrev)
                                == EOF) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            /* Read Soil Carbon Storage */
                            if (fscanf(init_state, " %lf",
                                       &cell[dist][veg][band].CLitter) == EOF) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            if (fscanf(init_state, " %lf",
                                       &cell[dist][veg][band].CInter) == EOF) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                            if (fscanf(init_state, " %lf",
                                       &cell[dist][veg][band].CSlow) == EOF) {
                                nrerror(
                                    "End of model state file found unexpectedly");
                            }
                        }
    ...
    if (options.LAKES) {
        if (options.BINARY_STATE_FILE) {
            ...
            for (dist = 0; dist < Ndist; dist++) {
                /* Read total soil moisture */
                for (lidx = 0; lidx < options.Nlayer; lidx++) {
                    if (fread(&lake_var->soil.layer[lidx].moist, sizeof(double),
                              1, init_state) != 1) {
                        nrerror("End of model state file found unexpectedly");
                    }
                }
