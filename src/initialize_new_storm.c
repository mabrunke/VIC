...
int
initialize_new_storm(cell_data_struct ***cell,
    ...
    ****************************************/
    if (veg < Nveg) {
        for (band = 0; band < options.SNOW_BAND; band++) {
            ...
            if (error) {
                fprintf(stderr,
                        "Wdew does not balance before new storm: %f -> %f record %i\n",
...
unsigned char
average_moisture_for_storm(double *wet_value,
                           double *dry_value,
                           double  old_mu,
                           double  new_mu)
{
...
**********************************************************************/
    ...
    *dry_value = temp_wet + temp_dry;
    diff =
        (temp_wet +
         temp_dry) - (*wet_value * new_mu + *dry_value * (1. - new_mu));
    if (fabs(diff) > 1.e-10) {
        error = TRUE;
    }
    else {
