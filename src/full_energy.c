    ...
    for (iveg = 0; iveg <= Nveg; iveg++) {
        ...
        if (veg_con[iveg].Cv > 0.0) {
            ...
            **************************************************/
            ...
            prepare_full_energy(iveg, Nveg, options.Nnode, prcp, soil_con,
                                moist0, ice0);
            ...
            *************************************/
            ...
            for (p = 0; p < N_PET_TYPES + 1; p++) {
                ...
                roughness[0] =
                    veg_lib[pet_veg_class].roughness[dmy[rec].month - 1];
            ...
            for (band = 0; band < Nbands; band++) {
                if (soil_con->AreaFract[band] > 0) {
                    cell[WET][iveg][band].aero_resist[0] =
                        aero_resist[N_PET_TYPES][0];
                    cell[WET][iveg][band].aero_resist[1] =
                ...
                for (dist = 0; dist < Ndist; dist++) {
                    for (band = 0; band < Nbands; band++) {
                        for (cidx = 0; cidx < options.Ncanopy; cidx++) {
                            veg_var[dist][iveg][band].rsLayer[cidx] =
                                HUGE_RESIST;
                        }
                        ...
                        if (dmy[rec].month == 1 && dmy[rec].day == 1) {
                            veg_var[dist][iveg][band].AnnualNPPPrev =
                                veg_var[dist][iveg][band].AnnualNPP;
                    ...
                    for (dist = 0; dist < Ndist; dist++) {
                        ...
                        for (lidx = 0; lidx < options.Nlayer; lidx++) {
                            if (veg_con->root[lidx] > 0) {
                                cell[dist][iveg][band].rootmoist +=
                                    cell[dist][iveg][band].layer[lidx].moist;
                            }
                            cell[dist][iveg][band].wetness +=
                                (cell[dist][iveg][band].layer[lidx].moist -
                                 soil_con->Wpwp[lidx]) /
    ...
    ****************************/
    ...
    if (options.LAKES && lake_con->lake_idx >= 0) {
                ...
                for (band = 0; band < Nbands; band++) {
                    if (soil_con->AreaFract[band] > 0) {
                        ...
                        for (dist = 0; dist < 2; dist++) {
                            ...
                            if (veg_con[iveg].LAKE) {
                                wetland_runoff +=
                                    (cell[dist][iveg][band].runoff * tmp_mu *
                                     Cv *
                                     soil_con->AreaFract[band]);
                                wetland_baseflow +=
                                    (cell[dist][iveg][band].baseflow * tmp_mu *
                                     Cv *
                                     soil_con->AreaFract[band]);
                                ...
                                sum_baseflow +=
                                    (cell[dist][iveg][band].baseflow * tmp_mu *
                                     Cv *
                                     soil_con->AreaFract[band]);
                                cell[dist][iveg][band].runoff *=
                                    (1 - lake_con->rpercent);
                                cell[dist][iveg][band].baseflow *=
                                    (1 - lake_con->rpercent);
                            }
                        }
                    }
        ...
        **********************************************************************/
        ...
        ErrorFlag = water_balance(lake_var, *lake_con, gp->dt, prcp, rec, iveg,
                                  band, lakefrac, *soil_con, *veg_con);
        if (ErrorFlag == ERROR) {
            return (ERROR);
        }
