...
int
get_depth_from_sarea(lake_con_struct lake_con,
                     double          sarea,
                     double         *depth)
/******************************************************************************
            ...
            if (sarea <= lake_con.basin[i] && sarea > lake_con.basin[i + 1]) {
                *depth =
                    ...
                    (lake_con.z[i] -
                     lake_con.z[i +
                                1]) /
                    (lake_con.basin[i] - lake_con.basin[i + 1]);
            }
        }
...
int
ice_depth(lake_con_struct lake_con,
          double          volume,
          double          ice_water_eq,
          double         *hice)
/******************************************************************************
        ...
        for (k = lake_con.numnod - 1; k >= 0; k--) {
            ...
            if (lake_con.z[k] > ldepth) {
                ...
                if (ldepth > lake_con.z[k + 1]) {
                    ...
                    if (tempvolume >=
                        ((lake_con.z[k] -
                          ldepth) * (lake_con.basin[k] + surfacearea) / 2.)) {
                        tempvolume -=
                            (lake_con.z[k] -
                             ldepth) * (lake_con.basin[k] + surfacearea) / 2.;
                    ...
                    /* Ice volume falls within this layer. */
                        ...
                        if (lake_con.basin[k] != lake_con.basin[k + 1]) {
                            ...
                            *hice +=
                                ((-1 *
                                  surfacearea) +
                                 sqrt(surfacearea * surfacearea + 2. * m *
                                      tempvolume)) / m;
                        }
                        else {
