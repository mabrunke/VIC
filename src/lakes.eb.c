    ...
    **********************************************************************/
    ...
    if (lake->activenod > 0 || lake->areai > 0.0) {
        ...
        **********************************************************************/
        ...
        if (fracprv >= 1.0) { /* areai is relevant */
            ...
            lake->ice_throughfall +=
                ((snowfall / 1000. + rainfall /
                  1000.) * (1 - fracprv) * lake->sarea);

            // If there is no snow, add the rain over ice directly to the lake.
            if (lake_snow->swq <= 0.0 && rainfall > 0.0) {
                lake->ice_throughfall +=
                    (rainfall / 1000.) * fracprv * lake->sarea;
                rainfall = 0.0; /* Because do not want it added to snow->surf_water */
            }
        }
        else {
            lake->ice_throughfall +=
                ((rainfall + snowfall) / 1000.) * lake->sarea;
            rainfall = 0.0;
            snowfall = 0.0;
        }
        ...
        **********************************************************************/
        ...
        else if (lake_snow->swq > 0. && lake_snow->swq <= SNOWCRIT * RHOSNOW /
                 RHO_W) {
        ...
        **********************************************************************/
        ...
        if ((1. - fracprv) > SMALL && lake->activenod > 0) {
            ...
            ErrorFlag = water_energy_balance(lake->activenod, lake->surface,
                                             ...
                                             lake->surfdz,
                                             (double)soil_con.lat, Tcutoff,
                                             tair, windw,
                                             pressure, vp, air_density, longin,
                                             sw_water,
                                             sumjoulb, wind_h, &Qhw, &Qew,
                                             &LWnetw, T,
                                             water_density,
                                             &lake_energy->deltaH,
        ...
        if (fracprv >= FRACLIM) {
            ...
            lake->aero_resist =
                (log((2. + soil_con.snow_rough) / soil_con.snow_rough) *
                 log(wind_h /
                     soil_con.snow_rough) / (von_K * von_K)) / windi;
...
// lake_energy->refreeze_energy += energy_ice_melt_bot;
            ...
            if (fracprv > 0.0) {
                energy_ice_melt_bot =
                    (lake->hice * RHOICE +
                     (snowfall / 1000.) * RHO_W) * Lf / (dt * SECPHOUR);
                lake->areai = 0.0;
                lake->hice = 0.0;
        ...
        **********************************************************************/
        ...
        if (lake->ice_water_eq > 0.0 && lake->new_ice_area > 0.0) {
            lake->hice =
                (lake->ice_water_eq / lake->new_ice_area) * RHO_W / RHOICE;
        }
        else {
            lake->hice = 0.0;
        }
...
**********************************************************************/
    ...
    *albi = 0.5 * albgs + 0.5 * albgl;
    ...
    if (swq > 0.0) {
        *snowalbedo = snow_albedo(newsnow, swq, depth, *snowalbedo, coldcontent,
...
**********************************************************************/
    ...
    **********************************************************************/
        ...
        **********************************************************************/
        ...
        ks = 6.6 * pow(sin((double)fabs(lat) * PI / 180.), 0.5) * pow(wind,
                                                                      -1.84);
        ws = 0.0012 * wind;
        Po = 1.0;
        ...
        for (k = 0; k < numnod - 1; k++) {
            ...
            **********************************************************************/
            ...
            if ((z * exp(ks * z) / ws) > 1.e8) {
                ...
                rad = 1. + 40. * N2 *
                      (von_K * z) * (von_K * z) / (ws * ws * exp(-2. * ks * z));
        ...
        if (volume - *ice_water_eq >= new_water_eq) {
            *ice_water_eq += new_water_eq;
            ...
            if ((volume - *ice_water_eq) > 0.0) {
                dibot =
                    (volume -
                     *ice_water_eq) / (sarea * (fracice) * RHOICE / RHO_W);
                *ice_water_eq = volume;
            }
            else {
                dibot = 0.0;
...
**********************************************************************/
    ...
    T1 = (sw_visible * (1 * surface_1 - surface_2 * exp(-lamwsw * surfdz)) +
          sw_nir *
          (1 * surface_1 - surface_2 * exp(-lamwlw * surfdz))) / surface_avg +
    ...
    *temph = 0.0;
    ...
    if (numnod == 1) {
        Tnew[0] = T[0] +
                  (T1 * dt *
                   SECPHOUR) / ((1.e3 + water_density[0]) * cp[0] * z[0]);
    }
    else {
        /* --------------------------------------------------------------------
        ...
        d[0] = T[0] +
               (T1 * dt *
                SECPHOUR) /
               ((1.e3 +
                 water_density[0]) * cp[0] * z[0]) + cnextra * dt * SECPHOUR;

        *energy_out_bottom =
            (surface_1 - surface_2) * (sw_visible * exp(-lamwsw * surfdz) +
                                       sw_nir *
                                       exp(-lamwlw * surfdz));
        ...
        for (k = 1; k < numnod - 1; k++) {
            ...
            T1 =
                (sw_visible *
                 (surface_1 *
                  exp(-lamwsw * top) - surface_2 * exp(-lamwsw * bot)) +
                 sw_nir *
                 (surface_1 *
                  exp(-lamwlw *
                      top) - surface_2 * exp(-lamwlw * bot))) / surface_avg;
            ...
            d[k] = T[k] +
                   (T1 * dt *
                    SECPHOUR) / ((1.e3 + water_density[k]) * cp[k] * z[k]) +
                   cnextra * dt * SECPHOUR;

            *energy_out_bottom +=
                (surface_1 - surface_2) * (sw_visible * exp(-lamwsw * bot) +
                                           sw_nir *
                                           exp(-lamwlw * bot));
        }
    ...
    *sumjoule = 0.0;
    ...
    for (k = 0; k < numnod; k++) {
        ...
        if (k == 0) {
            ...
            energy =
                (finaltemp[k] +
                 KELVIN) * dz * (1000. + density[k]) * cp[k] * surface[k] / 2.;
        }
        else {
            energy =
                (finaltemp[k] +
...
**********************************************************************/
{
    ...
    **********************************************************************/
    ...
    if (fabs(lake->evapw) > SMALL && lake->evapw >
        ((lake->volume -
    ...
    **********************************************************************/
    ...
    if (max_newfraction > lakefrac) {
        ...
        if (lake->volume - lake->ice_water_eq > lake->recharge) { // enough liquid water to support recharge
            ...
            Recharge = 1000. * lake->recharge /
                       ((max_newfraction -
                         lakefrac) *
                        lake_con.basin[0]) +
                       (veg_var[WET][iveg][band].Wdew +
                        snow[iveg][band].snow_canopy *
                        1000. + snow[iveg][band].swq * 1000.);                                                                                                                            // mm over area that has been flooded
    ...
    **********************************************************************/
    ...
    for (frost_area = 0; frost_area < options.Nfrost; frost_area++) {
        liq +=
            (soil_con.max_moist[lindex] -
             cell[WET][iveg][band].layer[lindex].ice[frost_area]) *
    ...
    if (ldepth <= lake_con.mindepth) {
        ...
        lake->runoff_out = lake_con.wfrac * circum * SECPHOUR * ((double)dt) *
    ...
    **********************************************************************/
    ...
    if (newfraction < 1.0) { // wetland exists at end of time step
        advect_soil_veg_storage(lakefrac, max_newfraction, newfraction,
                                ...
                                &(cell[WET][iveg][band]),
                                &(veg_var[WET][iveg][band]), lake_con);
        ...
        for (j = 0; j < options.Nlayer; j++) {
            moist[j] = cell[0][iveg][band].layer[j].moist;
        }
        ...
        if (lakefrac > 0.0) { // lake also existed at beginning of step
            for (j = 0; j < options.Nlayer; j++) {
                lake->evapw += cell[WET][iveg][band].layer[j].evap * 0.001 *
                               (1. - lakefrac) * lake_con.basin[0];
            }
            lake->evapw += veg_var[WET][iveg][band].canopyevap * 0.001 *
                           (1. - lakefrac) * lake_con.basin[0];
    ...
    if (newfraction > 0.0) { // lake exists at end of time step
        ...
        lake->soil.runoff = lake->runoff_out * 1000 /
                            (newfraction * lake_con.basin[0]);
        lake->soil.baseflow = lake->baseflow_out * 1000 /
                              (newfraction * lake_con.basin[0]);
        lake->soil.inflow = lake->baseflow_out * 1000 /
                            (newfraction * lake_con.basin[0]);
        for (lindex = 0; lindex < options.Nlayer; lindex++) {
            lake->soil.layer[lindex].evap = 0;
        }
        lake->soil.layer[0].evap += lake->evapw * 1000 /
                                    (newfraction * lake_con.basin[0]);
        ...
        if (lakefrac > 0.0) { // lake existed at beginning of time step
            ...
            initialize_lake(lake, lake_con, &soil_con, &(cell[WET][iveg][band]),
        ...
        if (lakefrac < 1.0) { // wetland also existed at beginning of step
            cell[WET][iveg][band].layer[0].evap += 1000. * lake->evapw /
                                                   ((1. -
                                                     newfraction) *
                                                    lake_con.basin[0]);
            cell[WET][iveg][band].runoff += 1000. * lake->runoff_out /
                                            ((1. -
                                              newfraction) * lake_con.basin[0]);
            cell[WET][iveg][band].baseflow += 1000. * lake->baseflow_out /
                                              ((1. -
                                                newfraction) *
                                               lake_con.basin[0]);
            cell[WET][iveg][band].inflow += 1000. * lake->baseflow_out /
                                            ((1. -
                                              newfraction) * lake_con.basin[0]);
        }
    }
    ...
    if (options.CARBON) {
        advect_carbon_storage(lakefrac, newfraction, lake,
                              &(cell[WET][iveg][band]));
    }
...
**********************************************************************/
{
    ...
    if (lakefrac < 1.0) { // wetland existed during this step
        ...
        if (delta_moist[0] > 0) {
            cell->baseflow += delta_moist[0] * 0.001 *
                              (1 - lakefrac) * lake_con.basin[0];            // m^3
            delta_moist[0] = 0;
        }
