...
static char vcid[] = "$Id$";
int
runoff(cell_data_struct  *cell_wet,
        ...
        if (mu > 0.) {
            for (lindex = 0; lindex < options.Nlayer; lindex++) {
                ...
                if (evap[lindex][0] != 0) { // if there is evaporation
                    ...
                    for (frost_area = options.Nfrost - 1;
                         frost_area >= 0;
                         frost_area--) {
                        evap[lindex][frost_area] =
                            avail_liq[lindex][frost_area] * evap_percent;
                        evap_sum -= evap[lindex][frost_area] *
                                    frost_fract[frost_area];
                    }
            ...
            for (lindex = 0; lindex < options.Nlayer; lindex++) {
                ...
                for (frost_area = 0; frost_area < options.Nfrost;
                    ...
                    if (options.Nfrost > 1) {
                        ...
                        Tlayer_spatial[lindex][frost_area] = linear_interp(
                            tmp_fract, 0, 1, min_temp, max_temp);
                    }
                    else {
            ...
            for (frost_area = 0; frost_area < options.Nfrost; frost_area++) {
                ...
                ******************************************************/
                ...
                compute_runoff_and_asat(soil_con, tmp_moist_for_runoff, inflow,
                                        &A, &(runoff[frost_area]));
                ...
                **************************************************/
                ...
                for (time_step = 0; time_step < dt; time_step++) {
                    ...
                    if (options.LOG_MATRIC) {
                        for (lindex = 0; lindex < options.Nlayer; lindex++) {
                            ...
                            if (tmp_liq > resid_moist[lindex]) {
                                matric[lindex] = soil_con->bubble[lindex] * pow(
                                    (tmp_liq -
                                     resid_moist[lindex]) /
                                    (soil_con->max_moist[lindex] -
                                     resid_moist[lindex]), -b[lindex]);
                            }
                            else {
                    ...
                    **************************************************/
                    ...
                    liq[lindex] +=
                        Q12[lindex -
                            1] - (evap[lindex][frost_area] + dt_baseflow);
