...
double
canopy_evap(layer_data_struct *layer_wet,
        ...
        if (mu > 0) {
            ...
            ****************************************************/
            ...
            rc = calc_rc((double)0.0, net_short, veg_lib[veg_class].RGL,
                         ...
                         (double)1.0, FALSE);
            canopyevap =
                pow((tmp_Wdew / veg_lib[veg_class].Wdmax[month - 1]),
                    (2.0 / 3.0)) *
...
**********************************************************************/
{
    ...
    *****************************************/
    ...
    if (layer[0].moist > veg_lib[veg_class].Wnpp_inhib * Wmax[0]) {
        *NPPfactor = veg_lib[veg_class].NPPfactor_sat +
                     (1 -
                      veg_lib[veg_class].NPPfactor_sat) *
                     (Wmax[0] -
                      layer[0].moist) /
                     (Wmax[0] - veg_lib[veg_class].Wnpp_inhib * Wmax[0]);
    }
    else {
        *NPPfactor = 1.0;
    }
    ...
    if (options.SHARE_LAYER_MOIST &&
        ...
        if (!options.CARBON || options.RC_MODE == RC_JARVIS) {
            ...
            *rc = calc_rc(veg_lib[veg_class].rmin, net_short,
            ...
            calc_rc_ps(veg_lib[veg_class].Ctype,
                       veg_lib[veg_class].MaxCarboxRate,
                       veg_lib[veg_class].MaxETransport,
                       veg_lib[veg_class].CO2Specificity,
                       NscaleFactor, air_temp, shortwave, aPARLayer, elevation,
                       Catm,
                       CanopLayerBnd, veg_lib[veg_class].LAI[month - 1],
                       gsm_inv, vpd, rsLayer, rc);
        }

        /* compute transpiration */
        evap = penman(air_temp, elevation, rad, vpd, ra, *rc,
                      veg_lib[veg_class].rarc) * delta_t / SEC_PER_DAY *
               dryFrac;
        ...
        for (i = 0; i < options.Nlayer; i++) {
            ...
            if (gsm_inv > 0.0) {
                ...
                if (!options.CARBON || options.RC_MODE == RC_JARVIS) {
                    ...
                    *rc = calc_rc(veg_lib[veg_class].rmin, net_short,
                    ...
                    calc_rc_ps(veg_lib[veg_class].Ctype,
                               ...
                               veg_lib[veg_class].MaxETransport,
                               veg_lib[veg_class].CO2Specificity,
                               NscaleFactor, air_temp, shortwave, aPARLayer,
                               elevation, Catm,
                               CanopLayerBnd, veg_lib[veg_class].LAI[month - 1],
                               gsm_inv, vpd, rsLayer, rc);
                }
                ...
                layerevap[i] = penman(air_temp, elevation, rad, vpd, ra, *rc,
                                      veg_lib[veg_class].rarc) * delta_t /
                               SEC_PER_DAY * dryFrac * (double)root[i];
